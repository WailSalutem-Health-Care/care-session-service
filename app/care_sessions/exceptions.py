...
class NFCTagNotFoundException(HTTPException):
    ...
    def __init__(self, tag_id: str):
        super().__init__(status_code=status.HTTP_404_NOT_FOUND, detail=f"NFC tag '{tag_id}' not found or inactive")
...
class InvalidSessionTimesException(HTTPException):
    ...
    def __init__(self):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail="Check-out time must be after check-in time")
...
class SessionNotInProgressException(HTTPException):
    ...
    def __init__(self, current_status: str):
        super().__init__(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Cannot complete session with status: {current_status}"
        )
...
class UnauthorizedCaregiverException(HTTPException):
    ...
    def __init__(self):
        super().__init__(status_code=status.HTTP_403_FORBIDDEN, detail="You can only complete your own sessions")
