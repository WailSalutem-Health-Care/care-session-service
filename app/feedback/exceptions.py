...
class FeedbackNotFoundException(HTTPException):
    ...
    def __init__(self, feedback_id: UUID):
        super().__init__(status_code=status.HTTP_404_NOT_FOUND, detail=f"Feedback with id {feedback_id} not found")
...
class FeedbackAlreadyExistsException(HTTPException):
    ...
    def __init__(self, session_id: UUID):
        super().__init__(
            status_code=status.HTTP_409_CONFLICT, detail=f"Feedback already exists for session {session_id}"
        )
...
class UnauthorizedFeedbackAccessException(HTTPException):
    ...
    def __init__(self):
        super().__init__(status_code=status.HTTP_403_FORBIDDEN, detail="You are not authorized to access this feedback")
