...
class FeedbackRepository(BaseRepository):
    ...
    async def get_daily_averages(self, start_date: date, end_date: date) -> List[Dict]:
        ...
        stmt = (
            ...
            .where(and_(date_col >= start_date, date_col <= end_date))
            .group_by(date_col)
            .order_by(date_col)
        )
    ...
    async def get_caregiver_weekly_feedbacks(
        ...
        stmt = (
            select(Feedback)
            .where(and_(Feedback.caregiver_id == caregiver_id, date_col >= week_start, date_col <= week_end))
    ...
    async def get_top_caregivers_of_week(self, week_start: date, week_end: date, limit: int = 3) -> List[Dict]:
        ...
        stmt = (
            ...
            .where(and_(date_col >= week_start, date_col <= week_end))
            .group_by(Feedback.caregiver_id)
    ...
    async def get_caregiver_average_rating(
        ...
        date_col = cast(Feedback.created_at, Date)
        stmt = select(
            func.avg(Feedback.rating).label("average_rating"), func.count(Feedback.id).label("total_feedbacks")
        ).where(and_(Feedback.caregiver_id == caregiver_id, date_col >= start_date, date_col <= end_date))
