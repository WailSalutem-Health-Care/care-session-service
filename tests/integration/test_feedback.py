...
async def test_create_feedback_unauthorized(client: AsyncClient):
    ...
    response = await client.post(
        "/feedback/", json={"session_id": str(session_id), "rating": 3, "patient_feedback": "Great service"}
    )
...
async def test_create_feedback_validation(
    ...
    response = await client.post(
        "/feedback/",
        json={"session_id": str(session_id), "rating": 5, "patient_feedback": "Test"},
    ...
    response = await client.post(
        "/feedback/",
        json={"session_id": str(session_id), "rating": 0, "patient_feedback": "Test"},
        headers={"Authorization": "Bearer mock_token"},
    )
    assert response.status_code == 422
...
async def test_list_feedback_structure(
    ...
    response = await client.get("/feedback/", headers={"Authorization": "Bearer mock_token"})
...
async def test_get_patient_average_rating(
    ...
    response = await client.get(
        f"/feedback/analytics/patient/{patient_id}/average", headers={"Authorization": "Bearer mock_token"}
    )
...
async def test_get_top_caregivers_weekly(
    ...
    response = await client.get(
        "/feedback/analytics/top-caregivers/weekly", headers={"Authorization": "Bearer mock_token"}
    )
...
async def test_daily_average_ratings(
    ...
    response = await client.get("/feedback/analytics/daily?days=7", headers={"Authorization": "Bearer mock_token"})
