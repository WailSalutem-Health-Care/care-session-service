...
async def test_create_care_session_unauthorized(client: AsyncClient):
    ...
    response = await client.post("/care-sessions/create", json={"tag_id": "test-tag-123"})
...
async def test_create_care_session_success(
    ...
    response = await client.post(
        "/care-sessions/create", json={"tag_id": "test-tag-123"}, headers={"Authorization": "Bearer mock_token"}
    )
...
async def test_list_care_sessions_structure(
    ...
    response = await client.get("/care-sessions/", headers={"Authorization": "Bearer mock_token"})
...
async def test_list_care_sessions_pagination(
    ...
    response = await client.get("/care-sessions/?page=1&page_size=10", headers={"Authorization": "Bearer mock_token"})
...
async def test_complete_care_session_unauthorized(client: AsyncClient):
    ...
    session_id = uuid4()
    response = await client.put(f"/care-sessions/{session_id}/complete", json={"caregiver_notes": "Test notes"})
    assert response.status_code in [401, 403]
