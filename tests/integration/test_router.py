...
def test_create_get_list_endpoints(fake_db, fake_jwt_payload, dummy_care_session):
    ...
    with patch.object(CareSessionService, "create_session", new=AsyncMock(return_value=dummy_care_session)):
        with patch.object(CareSessionService, "get_session", new=AsyncMock(return_value=dummy_care_session)):
            with patch.object(CareSessionService, "list_sessions", new=AsyncMock(return_value=([dummy_care_session], 1))):
                with patch.object(CareSessionService, "complete_session", new=AsyncMock(return_value=dummy_care_session)):
                    with patch.object(CareSessionService, "update_session", new=AsyncMock(return_value=dummy_care_session)):
                        with patch.object(CareSessionService, "delete_session", new=AsyncMock(return_value=True)):
                            ...
                            # Complete (PUT)
                            resp4 = client.put(f"/care-sessions/{dummy_care_session.id}/complete", json={"caregiver_notes": "done"})
                            assert resp4.status_code == 200

                            # Update (PATCH)
                            resp5 = client.patch(f"/care-sessions/{dummy_care_session.id}", json={"status": "completed"})
...
def test_error_cases_care_sessions(fake_db, fake_jwt_payload):
    ...
    with patch.object(CareSessionService, "get_session", new=AsyncMock(side_effect=CareSessionNotFoundException(uuid4()))):
        resp = client.get(f"/care-sessions/{uuid4()}")
        assert resp.status_code == 404

    # Test 409 for duplicate session creation
    with patch.object(CareSessionService, "create_session", new=AsyncMock(side_effect=DuplicateActiveSessionException())):
        resp = client.post("/care-sessions/create", json={"tag_id": "TAG1", "session_id": None})
    ...
    with patch.object(CareSessionService, "complete_session", new=AsyncMock(side_effect=CareSessionNotFoundException(uuid4()))):
