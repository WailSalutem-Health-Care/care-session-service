...
async def test_full_care_session_flow(client: AsyncClient, mock_jwt_payload):
    ...
    completed_session = create_mock_session(
        session_id, patient_id, status="completed", caregiver_notes="Patient visited successfully"
    )
    ...
    try:
        ...
        with patch("app.care_sessions.router.CareSessionService") as MockService:
            ...
            create_response = await client.post(
                "/care-sessions/create", json={"tag_id": tag_id}, headers={"Authorization": "Bearer mock_token"}
            )
            ...
            get_response = await client.get(
                f"/care-sessions/{session_id}", headers={"Authorization": "Bearer mock_token"}
            )
